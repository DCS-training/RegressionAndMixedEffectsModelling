...
Note that in R markdown, packages should be installed in the console (the 
...
summary(vocabdata$vocab_test_score)
summary(vocabdata$proficiency)
vocabdata 
...
want to learn more about contrast coding:
...
In other words, we need to build a model to inform us about how learners' vocab test 
...
Intercept. The model intercept represents the expected value of the outcome variable when the predictor variable is at 0. In this case, the model estimates that students are expected to have a score of 63.15
...
Slope. The coefficient of the predictor "week" is the slope. It represents on average, the number of units by which the outcome variable increases for a unit increase in the predictor variable. In this case, students' test 
...
hypothesis (which assumes the model explains nothing; i.e., the predictor has no effect on the outcome variable at all). The p-values indicates 
...
the predictor "week" does help explain the data (i.e., "week" does have an effect on test scores).
...
compared to the one-predictor model (m1); F(1) = 478.58, p < 0.001.
...
The one-predictor model explains 8% of the variances in the data whereas the two-predictor model does 60%. It is useful to have include the second predictor "proficiency" in the model. 
...
In other words, is the training course as effective for the 
high-proficient students compared to the intermediate-proficient students?

#### Visualise the data

We can use ggplot() for this.
...
The following two model structures are identical.

in R, `X1*X2` = `X1 + X2 + X1:X2`

The latter is just a simplified way of specifying both main effects (e.g., X1, 
...
x = 0. i.e., difference of the test scores between the two proficiency groups at week0. 
...
Is the interactive model better than the additive model? In other words, does 
...
Check the adjusted R-squared. We can see the additive model explained 60%
of the data whereas the interactive model explained 63%. The anova test suggests that this difference is significant.
...
one-predictor model (i.e., our first model that included only one predictor "week"). Yes, we can. However, whether it is a good approach needs some thinking. Let's try it out first.

Recall that our aim here is to understand how to best deal with individual difference
...
of "week" for now (leaving proficiency aside). With this in mind, we will build a model including "week" and "participant" as two predictor 
variables. This will fit a separate regression line for each participant. 
...
Think point: How useful do you think the model results are?
...
however, this is not very helpful for us our research goals. 
...
This leads us to linear MIXED-effects regression models, in which we consider both the FIXED and the RANDOM effects. In doing so, we can control for individual 
...
2. How does (3) differ from (2)?

You can very quickly run the mixed-effect models to get a bit of the taste. Feel free to use the code below.
